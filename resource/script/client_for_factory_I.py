...
def download_segBase(directory, dom):
	if not os.path.exists(directory):
		os.makedirs(directory)
	segBaseName = dom.getElementsByTagName("SegmentBase")[0].getElementsByTagName("Initialization")[0]
	segBaseFileName = str(segBaseName.attributes["sourceURL"].value)
	segBaseURL = folderUrlPath + '/' + segBaseFileName

	h = httplib2.Http(".cache")
	try:
		resp, content = h.request(segBaseURL, "GET")
		if resp.status ==200:
			pass
		else:
	...
	fileCacheMatch = ".cache/" + cacheMatch + "," + segBaseFileName+"*"
...
def play_video(videoName, width, height):
	...
	f = open(logName, "wb")
	p = subprocess.Popen(["mplayer", "-fps", "25", videoName, "-x", width, "-y", height, "-loop 0"], stdout=f, 
...
if(sys.argv[2]=="-play"):
	...
	segBaseFile = download_segBase(videoName, parseResult["data"])
	...
	for i in range(0, segNumber):
		...
		command.append(segBaseFile) # segment-base
		for j in fileList:
			command.append(j)
