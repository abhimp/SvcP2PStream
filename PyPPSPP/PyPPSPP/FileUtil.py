...
def calculate_hash(path):
    ...
    return str(binascii.hexlify(hash).decode("utf-8"))
...
def main(args):
    ...
    if args.hash and args.directory:
        hashListFile = args.directory + 'hash_list.txt'
        for root, dirs, files in os.walk(args.directory, topdown = False):
            for name in files:
                with open(hashListFile, "a") as myfile:
                    filename = str(os.path.join(args.directory, name))
                    myfile.write(name + '\t' + calculate_hash(filename) + '\t' + str(os.stat(filename).st_size) + '\n')
        return
