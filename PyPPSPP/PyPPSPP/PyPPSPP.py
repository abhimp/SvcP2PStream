...
if __name__ == "__main__":
    ...
    defaults['logger'] = r"C:\PyPPSPP\vod333.dat.log"
    ...
    parser.add_argument("--port", help="Port client is running on", nargs='?', type=int, default=defaults['port'])
    parser.add_argument("--logger", help="Configure log file", nargs='?', default=defaults['logger'])
    ...
    logging.basicConfig(level=logging.INFO, format='[%(levelname)s] %(asctime)s %(message)s', filename = args.logger)
    logging.info ("PyPPSPP starting")
